...
local function set_current_min_damage() -- sets the vis / invis sliders to the weapon's current min damage, so weapons min damages don't get set to 1 when you run this lua
	for i, v in ipairs(WEAPON_GROUPBOX_VARNAMES) do
		local current_min_dmg = gui.GetValue("rbot.accuracy.weapon." .. v .. ".mindmg")
...
callbacks.Register("Draw", function()
	...
	if index then
		if visible then
			gui.SetValue("rbot.accuracy.weapon." .. current_weapon_ui_name .. ".mindmg", min_damage_visible[index][1]:GetValue())
		else
			gui.SetValue("rbot.accuracy.weapon." .. current_weapon_ui_name .. ".mindmg", min_damage_invisible[index][1]:GetValue())
		end
	end
	
	if debug_draw_min_dmg:GetValue() then
		local current_weapon_min_dmg = gui.GetValue("rbot.accuracy.weapon." .. current_weapon_ui_name .. ".mindmg")
